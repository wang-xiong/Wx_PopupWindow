...
import android.app.LongSnapClientJobManager;
import android.app.ResolutionHelper;
...
import android.content.res.NubiaResources;
...
import android.util.PowerMoConfig;
...
import android.view.KeyguardMusicHelper;
...
//nubia add begin by liushitao
//nubia add end by liushitao
//NUBIA ADD begin for theme
//NUBIA ADD end
// PowerMo modified begin
// PowerMo modified end
//supersnap add begin
//supersnap add end
//Nubia Different Resolution Begin
//Nubia Different Resolution End
// NUBIA ADD for keyguard music
// NUBIA ADD end
...
//Nubia Different Resolution End
    ...
    //supersnap add begin
    private LongSnapClientJobManager  mLongSnapJobMgr = null;
    private boolean mLongSnapReceiverRegistered = false;
    private final int MAX_SNAP_PAGE_CNT = 6;
    //supersnap add end

    //Nubia Different Resolution Begin
    private int mSysDensityDpi = DisplayMetrics.DENSITY_DEVICE;
    //Nubia Different Resolution End
    ...
    // NUBIA ADD for keyguard music
    private KeyguardMusicHelper mKeyguardMusicHelper = KeyguardMusicHelper.getKeyguardMusicHelper();
    // NUBIA ADD end
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        public final void scheduleLaunchActivity(Intent intent, IBinder token, int ident,
            ...
            //Nubia Different Resolution Begin
            if(ResolutionHelper.getInstance().shouldReduceResolution("AT-SLA")) {
                if(curConfig != null) {
                    curConfig.densityDpi = ResolutionHelper.getInstance().getDensityDpi();
                }
            }
            if(DEBUG_RESOLUTION) {
                Slog.v(TAG," schedule Launch Activity  :"+curConfig);
            }
            //Nubia Different Resolution End
        ...
        public final void scheduleRelaunchActivity(IBinder token,
            ...
            //Nubia Different Resolution Begin
            if(ResolutionHelper.getInstance().shouldReduceResolution("AT-SRLA")) {
                if(config != null) {
                    config.densityDpi = ResolutionHelper.getInstance().getDensityDpi();
                }
                if(overrideConfig != null) {
                    overrideConfig.densityDpi = ResolutionHelper.getInstance().getDensityDpi();
                }
            }
            if(DEBUG_RESOLUTION) {
                Slog.v(TAG," schedule ReLaunch Activity  :"+config+"  overrideConfig:"+overrideConfig);
            }
            //Nubia Different Resolution End
        ...
        public final void bindApplication(String processName, ApplicationInfo appInfo,
            ...
            if(appInfo != null) {
                if(DEBUG_RESOLUTION) {
                    Slog.v(TAG," appInfo.overrideResolution:"+appInfo.overrideResolution);
                }
                if(appInfo.overrideResolution != 0) {
                    if(!ResolutionHelper.getInstance().setLocalResolution(appInfo.overrideResolution)) {
                        if(DEBUG_RESOLUTION) {
                            Slog.v(TAG," get resolution from appinfo fail:"+appInfo.overrideResolution);
                        }
                    }
                }
            }
        ...
        public void scheduleConfigurationChanged(Configuration config) {
            //Nubia Different Resolution Begin
            if(ResolutionHelper.getInstance().shouldReduceResolution("AT-SCC")) {
                mSysDensityDpi = config.densityDpi;
                if(config != null) {
                    config.densityDpi = ResolutionHelper.getInstance().getDensityDpi();
                }
            }
            if(DEBUG_RESOLUTION) {
                Slog.v(TAG," send CONFIGURATION_CHANGED  :"+config);
            }
            //Nubia Different Resolution End
...
// PowerMo modified begin
        public void scheduleSensorChanged(IBinder token, int value, int reserved) {
            if (!PowerMoConfig.WITHOUT_SMART_SENSOR) {
                SensorEventData data = new SensorEventData();
                data.token = token;
                data.value = value;
                data.reserved = reserved;
                sendMessage(H.SENSOR_CHANGED, data);
            }
        }
// PowerMo modified end
...
// PowerMo modified begin
            if (!PowerMoConfig.WITHOUT_SMART_CONFIGURATION) {
                if (args.length > 1 && args[1].equals("res")) {
                    FileOutputStream fout = new FileOutputStream(fd);
                    PrintWriter pw = new FastPrintWriter(fout);
                    try {
                        pw.println("---Resource information---");
                        pw.println("  mConfiguration=" + mConfiguration);
                        pw.println("  mCompatConfiguration=" + mCompatConfiguration);
                        pw.println("  mPendingConfiguration=" + mPendingConfiguration);
                        //mResourcesManager.dump(pw);
                    } finally {
                        pw.flush();
                    }
                }
...
// PowerMo modified end
        ...
        //NUBIA ADD for anr log
        public void dumpMessageHistory(){
            LogPrinter lp = new LogPrinter(Log.DEBUG, android.os.NubiaDebug.TAG_NUBIA);
            lp.println("dumpMessageHistory in mainLooper");
            Looper.getMainLooper().dumpMessageHistory(lp,"  ");
        }
        //NUBIA END

        ////NUBIA ADD for process freezer
        @Override
        public void scheduleFreeze() {
        }

        @Override
        public void scheduleUnfreeze(boolean isOneWay) {
        }
        //NUBIA ADD END
        // NUBIA ADD for keyguard music
        public void scheduleControlMusic(int command) {
            if (KeyguardMusicHelper.KEYGUARD_MUSIC) {
                try {
                    if (mKeyguardMusicHelper != null) {
                        mKeyguardMusicHelper.scheduleControlMusic(command);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
...
// PowerMo modified begin
        public static final int SENSOR_CHANGED          = 200;
// PowerMo modified end
...
// PowerMo modified begin
                    case SENSOR_CHANGED: return "SENSOR_CHANGED";
// PowerMo modified end
                }
            }
...
// PowerMo modified begin
                case SENSOR_CHANGED:
                    Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "SensorChanged");
                    SensorEventData sed = (SensorEventData)msg.obj;
                    handleSensorChanged(sed.token, sed.value, sed.reserved);
                    Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
                    break;
// PowerMo modified end
            }
            ...
            if (a != null) {
                ...
                do {
                    ...
                    if (a.activity != null && !a.activity.mFinished) {
                        try {
// PowerMo modified begin
                            if (!PowerMoConfig.WITHOUT_SMART_CONFIGURATION && a.createdConfig != null && a.activity.mCurrentConfig != null
                                    && a.createdConfig.isOtherSeqNewer(a.activity.mCurrentConfig)) {
                                 //Nubia Different Resolution Begin
                                 if(ResolutionHelper.getInstance().shouldReduceResolution("AT-QI")) {
                                    if(a.activity.mCurrentConfig != null) {
                                        a.activity.mCurrentConfig.densityDpi = ActivityThread.this.mSysDensityDpi;
                                    }
                                }
                                //Nubia Different Resolution End
                                am.activityIdle(a.token, a.activity.mCurrentConfig, stopProfiling);
                            } else {
                                 //Nubia Different Resolution Begin
                                 if(ResolutionHelper.getInstance().shouldReduceResolution("AT-QI")) {
                                    if(a.createdConfig != null) {
                                        a.createdConfig.densityDpi = ActivityThread.this.mSysDensityDpi;
                                    }
                                }
                                //Nubia Different Resolution End
...
// PowerMo modified end
    ...
    Resources getTopLevelResources(String packageName, String resDir,
            String[] splitResDirs, String[] overlayDirs, String[] libDirs,
            int displayId, LoadedApk pkgInfo) {
        Resources r = mResourcesManager.getResources(null, resDir,
                splitResDirs, overlayDirs, libDirs, displayId, null,
                pkgInfo.getCompatibilityInfo(), pkgInfo.getClassLoader());
        if (r != null) {
            ((NubiaResources) r).init(packageName);
        }
        return r;
    }
    // NUBIA ADD end
    ...
    //supersnap add begin
    private void registerLongSnapReceiver(Context context) {
        IntentFilter filter = new IntentFilter();
        filter.addAction("cn.nubia.action.longsnap");
        context.registerReceiver(new LongSnapReceiver(), filter);
    }

    private class LongSnapReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
           //String  expectPkgName = intent.getExtra("pkgname");
           Log.i(TAG, "LongSnapReceiver intent:"+intent);
            Activity activity = findResumedActivity();
            Log.i(TAG, "LongSnapReceiver activity:"+activity);
            if (activity != null) {
                View decorView = null;
                Log.i(TAG, "decorView = " + decorView);
                ArrayList<ViewRootImpl> views = WindowManagerGlobal.getInstance().getRootViews();
                for (ViewRootImpl viewRoot : views) {
                    Log.i(TAG, "view Root = " + viewRoot.getView() + ", hasWindowFocus : " + viewRoot.getView().hasWindowFocus());
                    if (viewRoot.getView() != null && viewRoot.getView().hasWindowFocus()) {
                        decorView = viewRoot.getView();
                    }
                }
                mLongSnapJobMgr.startOneJob(activity.getApplicationContext(), decorView, intent);
            }
        }
    }
    ...
    private Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        try {
            ...
            //supersnap add begin
             if (!mLongSnapReceiverRegistered) {
                Log.d(TAG, "registerLongSnapReceiver for " + app);
                WindowManager wm = (WindowManager)app.getSystemService(Context.WINDOW_SERVICE);
                int dispWidth = wm.getDefaultDisplay().getWidth();
                int dispHeight = wm.getDefaultDisplay().getHeight();
                mLongSnapJobMgr = new LongSnapClientJobManager(dispWidth, dispHeight,
                                                               MAX_SNAP_PAGE_CNT);
                registerLongSnapReceiver(app);
                mLongSnapReceiverRegistered = true;
            }
            //supersnap add end
            ...
            if (activity != null) {
                ...
                if (KeyguardMusicHelper.KEYGUARD_MUSIC) {
                    try {
                        if (mKeyguardMusicHelper != null) {
                            mKeyguardMusicHelper.shouldShowOnKeyguard(activity);
                            if (mKeyguardMusicHelper.checkShouldShowOnKeyguard(activity)) {
                                mKeyguardMusicHelper.registerActivity(activity);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    private void handleLaunchActivity(ActivityClientRecord r, Intent customIntent, String reason) {
        ...
        if (KeyguardMusicHelper.KEYGUARD_MUSIC) {
            try {
                if (mKeyguardMusicHelper != null) {
                    if (mKeyguardMusicHelper.checkShouldShowOnKeyguard(r.activity)) {
                        if (!mKeyguardMusicHelper.checkShouldLaunchActivity()) {
                            try {
                                ActivityManagerNative.getDefault().finishActivity(
                                        r.token, Activity.RESULT_CANCELED, null,
    ...
    private void handleReceiver(ReceiverData data) {
        ...
        try {
            ...
            // NUBIA ADD for anr log
            final long now = SystemClock.uptimeMillis();
            final long delay = now - start;
            if (delay >= android.os.NubiaDebug.RECEIVER_DELAY) {
                Log.i(android.os.NubiaDebug.TAG_NUBIA, "ActivityThread onReceive() delay:" + delay+ "ms receiver:" + receiver + " intent:"+ data.intent);
            }
            // NUBIA ADD END
        } catch (Exception e) {
            if (DEBUG_BROADCAST) Slog.i(TAG,
    ...
    private void handleCreateService(CreateServiceData data) {
        ...
        try {
            ...
            final long now = SystemClock.uptimeMillis();
            final long delay = now - start;
            if (delay >= android.os.NubiaDebug.SERVICE_DELAY) {
                Log.i(android.os.NubiaDebug.TAG_NUBIA, "ActivityThread service.onCreate() delay:" + delay+ "ms service:" + service + " data:"+ data);
            }
            // NUBIA ADD END
            ...
            try {
                ...
                try {
                    if (!data.rebind) {
                        ...
                        final long delay = now - start;
                        if (delay >= android.os.NubiaDebug.SERVICE_DELAY) {
                            Log.i(android.os.NubiaDebug.TAG_NUBIA, "ActivityThread service.onBind() delay:" + delay+ "ms service:" + s + " data:"+ data);
                        }
                        // NUBIA ADD END
                        ...
                        final long delay = now - start;
                        if (delay >= android.os.NubiaDebug.SERVICE_DELAY) {
                            Log.i(android.os.NubiaDebug.TAG_NUBIA, "ActivityThread service.onRebind() delay:" + delay+ "ms service:" + s + " data:"+ data);
                        }
                        // NUBIA ADD END
                        ActivityManagerNative.getDefault().serviceDoneExecuting(
                                data.token, SERVICE_DONE_EXECUTING_ANON, 0, 0);
    ...
    private void handleUnbindService(BindServiceData data) {
        ...
        if (s != null) {
            try {
                ...
                final long delay = now - start;
                if (delay >= android.os.NubiaDebug.SERVICE_DELAY) {
                    Log.i(android.os.NubiaDebug.TAG_NUBIA, "ActivityThread service.onUnbind() delay:" + delay+ "ms service:" + s + " data:"+ data);
                }
                // NUBIA ADD END
                try {
                    if (doRebind) {
        ...
        if (s != null) {
            try {
                ...
                if (!data.taskRemoved) {
                    ...
                    final long delay = now - start;
                    if (delay >= android.os.NubiaDebug.SERVICE_DELAY) {
                        Log.i(android.os.NubiaDebug.TAG_NUBIA, "ActivityThread service.onStartCommand() delay:" + delay+ "ms service:" + s + " data:"+ data);
                    }
                    // NUBIA ADD END
                } else {
                    // NUBIA ADD for anr log
                    ...
                    final long delay = now - start;
                    if (delay >= android.os.NubiaDebug.SERVICE_DELAY) {
                        Log.i(android.os.NubiaDebug.TAG_NUBIA, "ActivityThread service.onTaskRemoved() delay:" + delay+ "ms service:" + s + " data:"+ data);
                    }
                    // NUBIA ADD END
    ...
    private void handleStopService(IBinder token) {
        ...
        if (s != null) {
            try {
                ...
                final long delay = now - start;
                if (delay >= android.os.NubiaDebug.SERVICE_DELAY) {
                    Log.i(android.os.NubiaDebug.TAG_NUBIA, "ActivityThread service.onDestroy() delay:" + delay+ "ms service:" + s);
                }
                // NUBIA ADD END
        ...
        if (r != null) {
            ...
            if (r.window == null && !a.mFinished && willBeVisible) {
                ...
                if (KeyguardMusicHelper.KEYGUARD_MUSIC) {
                    try {
                        if (mKeyguardMusicHelper != null) {
                            if (KeyguardMusicHelper.DEBUG) {
                                Log.d(KeyguardMusicHelper.TAG, "handleResumeActivity");
                            }
                            if (mKeyguardMusicHelper.checkShouldShowOnKeyguard(r.activity)) {
                                mKeyguardMusicHelper.changeActivityWindowType(r.activity);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    private void handleDestroyActivity(IBinder token, boolean finishing,
        ...
        if (KeyguardMusicHelper.KEYGUARD_MUSIC) {
            try {
                if (mKeyguardMusicHelper != null) {
                    if (mKeyguardMusicHelper.checkShouldShowOnKeyguard(r.activity)) {
                        mKeyguardMusicHelper.unregisterActivity();
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void handleBindApplication(AppBindData data) {
        ...
        //Nubia Different Resolution Begin
        if(data.config != null && ResolutionHelper.getInstance().shouldReduceResolution("AT-HBA")) {
            mSysDensityDpi = data.config.densityDpi;
            if(data.config != null) {
                data.config.densityDpi = ResolutionHelper.getInstance().getDensityDpi();
            }
        }
        if(DEBUG_RESOLUTION) {
            Slog.i(TAG, " handleBindApplication config:"+data.config);
        }
        //Nubia Different Resolution End
        ...
        //Nubia add for preStart  NOTFINISHWHENBACK
        if (data.debugMode>=100) {
            int backToHomeFlag = data.debugMode/100;
            com.android.internal.policy.PhoneWindow.setBackToHome(true, backToHomeFlag==2);
            data.debugMode -= backToHomeFlag*100;
        }
        boolean isPreStart = data.debugMode>=10;
        if (isPreStart) {
            data.debugMode -= 10;
        }
        //Nubia add end
...
// PowerMo modified begin
            String enable = SystemProperties.get("persist.smartshow.context.debug", null);
            if(enable != null && enable.equals("1")) {
                try {
                    mInstrumentation = (Instrumentation)ClassLoader.getSystemClassLoader()
                        .loadClass(SMARTSHOW_INSTRUMENTATION).newInstance();
                } catch (Exception e) {
                }
            }
// PowerMo modified end
        ...
        try {
            ...
            try {
                ...
                //Nubia add for preStart
                if (isPreStart) app.onPreLaunch();
                //Nubia add end
            } catch (Exception e) {
...
// PowerMo modified begin
        /*
        if (!PowerMoConfig.WITHOUT_ALL) {
            try {
                ISmartShowContext context = (ISmartShowContext)ClassLoader.getSystemClassLoader().
                        loadClass(SMARTSHOW_CONTEXTPROXY).newInstance();
                SmartShowContext.attach(context);
            } catch (Exception e) {
                throw new RuntimeException("Create SmartShowContext Proxy Error", e);
            }
        }
        */
// PowerMo modified end
...
// PowerMo modified begin
    public static final String SMARTSHOW_CONTEXTPROXY= "com.powermo.smartshow.frameworks.SmartShowContextProxy";
    public static final String SMARTSHOW_INSTRUMENTATION = "com.powermo.smartshow.frameworks.MInstrumentation";
    /** @hide */
    static final class SensorEventData {
        IBinder token;
        int value;
        int reserved;
    }
    ...
    /** @hide */
    final void handleSensorChanged(IBinder token, int value, int reserved) {
        if (token == null) {
            /* PowerMo comment
             * It means we should apply it for all contexts.
             */
            final int NAPP = mAllApplications.size();
            for (int i =0; i<NAPP; i++) {
                Application ap = mAllApplications.get(i);
                if (ap != null) {
                    try {
                        ((ContextImpl)ap.getBaseContext()).updateSensorRotation(value, true);
                    } catch (Exception e) {
                    }
                }
            }
            ...
            for (int i=0; i<NACT; i++) {
                ...
                if (a != null) {
                    try {
                        ((ContextImpl)a.getBaseContext()).updateSensorRotation(value, true);
                    } catch (Exception e) {
                    }
                }
            }
            final int NSVC = mServices.size();
            for (int i=0; i<NSVC; i++) {
                Service s = mServices.valueAt(i);
                try {
                    ((ContextImpl)s.getBaseContext()).updateSensorRotation(value, true);
                } catch (Exception e) {
                }
            }
            return;
        }
        ContextImpl baseContext = null;
        ActivityClientRecord r = mActivities.get(token);
        if(r != null) {
            if(r == null || r.activity == null) {
                return;
            ...
            try {
                baseContext = (ContextImpl)r.activity.getBaseContext();
            } catch (Exception e) {
                baseContext = null;
                return;
            }
            ...
            try {
                baseContext = (ContextImpl)s.getBaseContext();
            } catch (Exception e) {
                baseContext = null;
                return;
            }
        }
        baseContext.updateSensorRotation(value, false);
    }
// PowerMo modified end
}
